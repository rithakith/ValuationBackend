...
namespace ValuationBackend.Migrations
{
    ...
    public partial class ConvertRentalEvidenceLAMasterFileIdToInt : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // Also ensure that the MasterFileId references an existing record in LandAquisitionMasterFiles
            migrationBuilder.Sql(@"
                UPDATE ""RentalEvidencesLA""
                SET ""MasterFileId_Temp"" = CASE 
                    WHEN ""MasterFileId"" ~ '^[0-9]+$' AND ""MasterFileId"" != '' THEN 
                        CASE 
                            WHEN EXISTS (SELECT 1 FROM ""LandAquisitionMasterFiles"" WHERE ""Id"" = CAST(""MasterFileId"" AS integer)) THEN
                                CAST(""MasterFileId"" AS integer)
                            ELSE 
                                (SELECT MIN(""Id"") FROM ""LandAquisitionMasterFiles"") -- Use the first available ID
                        END
                    ELSE 
                        (SELECT MIN(""Id"") FROM ""LandAquisitionMasterFiles"") -- Default to first available ID
                END;
            ");
